...
impl<'input> Parser<'input> {
    pub fn parse(txt: &'input str, ctx: &mut Ctx) -> Res<()> {
        ...
        match slf.top(ctx) {
            Ok(res) => Ok(res),
            Err(mut err) => {
                err = err.with_context(slf.position());
                if !slf.at_eoi() {
                    err = err.context(|| {
                ...
                Err(err.with_context("parsing failed"))
            }
        }
    }
...
impl<'input> Parser<'input> {
    ...
    /// Skips an XML comment if present (`<!-- ... -->`). Returns true if a comment was skipped.
    pub fn skip_comment(&mut self) -> bool {
        if self.tail().starts_with("<!--") {
            ...
            while !self.at_eoi() {
                ...
                let c = self.tail().chars().next().unwrap();
                if c == '\n' {
                    self.line += 1;
                    self.column = 1;
                } else {
    ...
    /// Skips whitespace and XML comments.
    pub fn ws_and_comments(&mut self) {
        loop {
            self.ws();
...
impl<'input> Parser<'input> {
    pub fn at_path(&mut self, ctx: &mut PathCtx) -> Res<()> {
        'walk: loop {
            ...
            if self.try_tag("<ecore:EPackage") {
                ...
                // Parse package content (annotations, classifiers, sub-packages)
                self.package_content(ctx)?;
                continue 'walk;
            }
            ...
            bail!("unexpected content at package level")
        }
    }
    ...
    pub fn enter_package(&mut self, ctx: &mut PathCtx) -> Res<()> {
        ...
        // Set nsURI and nsPrefix if they were provided
        if let Some(uri) = ns_uri {
            ctx.current_pack_mut().set_ns_uri(uri);
        }
        if let Some(prefix) = ns_prefix {
            ctx.current_pack_mut().set_ns_prefix(prefix);
        }

        Ok(())
    }

    pub fn package_content(&mut self, ctx: &mut PathCtx) -> Res<()> {
        'content: loop {
            self.ws_and_comments();
            ...
            if self.try_tag("<eAnnotations") {
                ...
                let annot = self
                    .annotation()
                    .with_context("failed to parse package annotation")?;
                ctx.add_annotation(annot);
                continue 'content;
            } else if self.try_tag("<ecore:EPackage") {
                self.ws();
                self.enter_package(ctx)?;
                self.package_content(ctx)?;
                continue 'content;
                ...
                bail!("unexpected package content")
            }
        }
        Ok(())
    }
    ...
    pub fn class_operation(&mut self, ctx: &mut ClassCtx) -> Res<repr::Operation> {
        ...
        'attributes: loop {
            ...
            match key {
                ...
                "lowerBound" => {
                    self.handle_redef("operation lower bound", "lowerBound", lbound.as_ref(), val)?;
                    lbound = Some(val);
                }
                "upperBound" => {
    ...
    pub fn class_structural(&mut self, ctx: &mut ClassCtx) -> Res<repr::Structural> {
        ...
        'attributes: loop {
            ...
            match (&*pref, key) {
                ...
                ([], "ordered") => {
                    self.handle_redef("class", "ordered", ordered.as_ref(), val)?;
                    ordered = Some(val);
                }
                ([], "changeable") => {
                    self.handle_redef("class", "changeable", changeable.as_ref(), val)?;
                    changeable = Some(val);
                }
                ([], "volatile") => {
                    self.handle_redef("class", "volatile", volatile.as_ref(), val)?;
                    volatile = Some(val);
                }
                ([], "transient") => {
                    self.handle_redef("class", "transient", transient.as_ref(), val)?;
                    transient = Some(val);
                }
                ([], "derived") => {
                    self.handle_redef("class", "derived", derived.as_ref(), val)?;
                    derived = Some(val);
                }
                ([], "unique") => {
        ...
        if let Some(ordered) = ordered {
            structural.set_ordered(helpers::bool(ordered)?);
        }
        if let Some(changeable) = changeable {
            structural.set_changeable(helpers::bool(changeable)?);
        }
        if let Some(volatile) = volatile {
            structural.set_volatile(helpers::bool(volatile)?);
        }
        if let Some(transient) = transient {
            structural.set_transient(helpers::bool(transient)?);
        }
        if let Some(derived) = derived {
            structural.set_derived(helpers::bool(derived)?);
        }
        ...
        if early_done {
            'content: loop {
                ...
                if self.try_raw_tag("</eStructuralFeatures>") {
                    break 'content;
                }
