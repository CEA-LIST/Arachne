...
impl Path {
    ...
    pub fn iter<'me>(&'me self) -> impl Iterator<Item = idx::Pack> + 'me {
        self.path.iter().cloned().chain(Some(self.last))
    }
    ...
    pub fn first(&self) -> idx::Pack {
        self.path.first().cloned().unwrap_or(self.last)
    }
...
impl Operation {
    ...
    pub fn new(name: impl Into<String>, typ: Option<idx::Class>, bounds: Bounds) -> Self {
        Self::with_capacity(name, typ, bounds, 3)
    }
...
impl Pack {
    ...
    pub fn set_ns_uri(&mut self, ns_uri: impl Into<String>) {
        self.ns_uri = Some(ns_uri.into());
    }

    pub fn ns_prefix(&self) -> Option<&str> {
        self.ns_prefix.as_deref()
    }
    pub fn set_ns_prefix(&mut self, ns_prefix: impl Into<String>) {
        self.ns_prefix = Some(ns_prefix.into());
    }
